...
description: Review the changes since a fixed point (commit, branch, tag, or merge-base) along two axes — Conventions (does the code follow this repo's documented coding conventions?) and Spec (does the code match what the originating issue/PRD asked for?). Runs both reviews in parallel sub-agents and reports them side by side. Use when the user wants to review a branch, a PR, work-in-progress changes, or asks to "review since X".
---

Two-axis review of the diff between `HEAD` and a fixed point the user supplies:

- **Conventions** — does the code conform to this repo's documented coding conventions?
...
2. If nothing is found, ask the user where the spec is. If they say there isn't one, the **Spec** sub-agent will skip
   and report "no spec available".

### 3. Identify the conventions sources

Anything in the repo that documents how code should be written, such as `CONTRIBUTING.md` or `docs/conventions/`.

On top of whatever the repo documents, the Conventions axis always carries the **smell baseline** below — a fixed set of
...
- **Always a judgement call.** Each smell is a labelled heuristic ("possible Feature Envy"), never a hard violation —
  and, like any standard here, skip anything tooling already enforces.

Each smell reads *what it is* → *how to fix*; match it against the diff:

- **Mysterious Name** — a function, variable, or type whose name doesn't reveal what it does or holds. → rename it; if
  no honest name comes, the design's murky.
...
- **Primitive Obsession** — a primitive or string standing in for a domain concept that deserves its own type. → give
...
Send a single message with two `Agent` tool calls, invoking the `conventions-reviewer` and `spec-reviewer`
custom agents respectively.

**Conventions sub-agent prompt** — include:

- The full diff command and commit list.
- The list of conventions-source files you found in step 3, **plus the smell baseline from step 3** pasted in full — the
  sub-agent has no other access to it.
- The brief: "Report — per file/hunk where relevant — (a) every place the diff violates a documented convention: cite the
  convention (file + the rule); and (b) any baseline smell you spot: name it and quote the hunk. Distinguish hard
  violations from judgement calls — documented-convention breaches can be hard, but baseline smells are always judgement
  calls, and a documented repo convention overrides the baseline. Skip anything tooling enforces. Under 400 words."
...
Present the two reports under `## Conventions` and `## Spec` headings, verbatim or lightly cleaned. Do **not** merge or
rerank findings — the two axes are deliberately separate (see _Why two axes_).
...
- Code that follows every convention but implements the wrong thing → **Conventions pass, Spec fail.**
- Code that does exactly what the issue asked but breaks the project's conventions → **Spec pass, Conventions fail.**
