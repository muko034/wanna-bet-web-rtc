...
- [ ] Opening the app on `/room/<code>` or `/room/<code>/play` when this device holds a snapshot for `<code>` resumes
      it with no prompt: a Lobby snapshot lands on the Lobby, a started game on `/room/<code>/play`.
- [ ] Resuming restores the full Game State and the original Room Code.
- [ ] While the Room's Transport ID is still held by the previous tab, the Host sees a "Reopening your Room…" screen
